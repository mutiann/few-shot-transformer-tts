...
def find_ckpt(model_path, map_location={}):
    import glob
    max_step = 0
    ckpt_file = None
    for f in glob.iglob(os.path.join(model_path, 'checkpoint_transformer_*.pth.tar')):
        filename = os.path.split(f)[-1]
        step = filename.split('.')[0].split('_')[-1]
        if int(step) > max_step:
            max_step = int(step)
            ckpt_file = f
    if ckpt_file is None:
        return None
    state_dict = t.load(ckpt_file, map_location=map_location)
    state_dict['step'] = max_step
    return state_dict
...
def plot_attn(attn, path, enc_length=None, dec_length=None):
    ...
    for k, layer_attn in enumerate(attn):
        layer_attn = layer_attn.cpu().detach().numpy()
        if enc_length:
