# Available at https://sites.google.com/site/shinnosuketakamichi/research-topics/jvs_corpus
...
filter_sub = [r'falset10', r'whisper10']

base_path = os.path.join(dataset_path, 'jvs_ver1')

spk_samples = defaultdict(list)
lang_name = 'ja_jp'
output_path = os.path.join(transformed_path, 'jvs')
wav_output_path = os.path.join(output_path, 'wavs')
os.makedirs(wav_output_path, exist_ok=True)
n_skip = 0

trans = list(glob.iglob(os.path.join(base_path, '**', 'transcripts_utf8.txt'), recursive=True))
...
for f in tqdm.tqdm(trans):
    ...
    for l in sub_samples:
        ...
        wav_file = os.path.join(f[:-len(transname)], 'wav24kHz16bit', filename + '.wav')
