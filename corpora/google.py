...
def process(base_path):
    ...
    samples = open(os.path.join(base_path, "line_index.tsv"), encoding='utf-8').read().splitlines()
    n_skip = 0
    print(lang, '\n', samples[0], '\n', samples[1], '\n', samples[2])
    ...
    for sample in tqdm.tqdm(samples):
        ...
        if not dur <= 20 or any([c in "1234567890" for c in script]):
            if n_skip < 10:
                print(dur, script)
            n_skip += 1
            continue
